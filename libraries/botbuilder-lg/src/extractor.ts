...
export class Extractor
    extends AbstractParseTreeVisitor<Map<string, unknown>>
    implements LGTemplateParserVisitor<Map<string, unknown>> {
     ...
     */
    public extract(): Map<string, unknown>[] {
        const result: Map<string, unknown>[] = [];
        this.templates.forEach((template: Template): void => {
            const templateResult: Map<string, unknown> = new Map<string, unknown>();
            ...
            templateBodies.forEach(
                (templateBody: Map<string, unknown>): boolean =>
     ...
     */
    public visitNormalTemplateBody(context: lp.NormalTemplateBodyContext): Map<string, unknown> {
        const result: Map<string, unknown> = new Map<string, unknown>();
     ...
     */
    public visitStructuredBody(context: lp.StructuredBodyContext): Map<string, unknown> {
        const result: Map<string, unknown> = new Map<string, unknown>();
        const lineStart = '    ';
     ...
     */
    public visitIfElseBody(context: lp.IfElseBodyContext): Map<string, unknown> {
        const result: Map<string, unknown> = new Map<string, unknown>();
        ...
        for (const ifRule of ifRules) {
            ...
            const templateBodies: Map<string, unknown> = this.visit(ifRule.normalTemplateBody());
            for (const templateBody of templateBodies) {
     ...
     */
    public visitSwitchCaseBody(context: lp.SwitchCaseBodyContext): Map<string, unknown> {
        const result: Map<string, unknown> = new Map<string, unknown>();
        ...
        for (const iterNode of switchCaseNodes) {
            ...
            const conditionLabel: string = node.text.toLowerCase();
            const childTemplateBodyResult: string[] = [];
            const templateBodies: Map<string, unknown> = this.visit(iterNode.normalTemplateBody());
            for (const templateBody of templateBodies) {
                childTemplateBodyResult.push(templateBody[0]);
            }
     ...
     */
    protected defaultResult(): Map<string, unknown> {
